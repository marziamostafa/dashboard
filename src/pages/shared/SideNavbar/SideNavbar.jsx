...
export default function SideNavbar({ isOpen }) {
  ...
  return (
    <div className="bg-white border-r border-[#cbd5e6] shadow-sm h-screen overflow-y-auto p-4 text-black">
      ...
      {/* Logo */}
      <div className={`border-b border-[#cbd5e6] text-black p-5 text-center ${!isOpen && "hidden"}`}>
        Logo
      </div>

      {menuItems.map((section, index) => (
        <div key={index} className="my-6">
          {isOpen && <p className="text-gray-500 text-xs uppercase mb-2">{section.section}</p>}
            ...
            {section.items.map((item, i) => (
              <li key={i}>
                {item.subItems ? (
                  <div>
                    {isOpen && (
                      ...
                      >
                        ...
                        <span>{openMenus[item.title] ? <IoIosArrowDown /> : <MdArrowForwardIos />}</span>
                      </button>
                    )}
                  </div>
                ) : (
                  <Link
                    to={item.path}
                    className={`flex items-center px-3 py-2 rounded hover:bg-gray-100 ${!isOpen && "justify-center"}`}
                  >
